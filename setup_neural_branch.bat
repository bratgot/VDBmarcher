...
:: Source — replaces existing VDBRenderIop.cpp with all neural changes applied
if exist "%NEURAL_SRC%\VDBRenderIop.cpp" (
    copy /Y "%NEURAL_SRC%\VDBRenderIop.cpp" "%REPO%\VDBRenderIop.cpp" >nul
    echo       Updated: VDBRenderIop.cpp
)
...
git commit -m "feat: NeuralVDB integration — neural compressed volume support" ^
           ...
           -m "  VDBRenderIop.cpp     — all BoxSampler density/shadow calls replaced" ^
...
echo.
echo   Next steps:
echo     1. Review INTEGRATION.md for a summary of all changes
echo     2. Run build_neural.bat to compile (with or without LibTorch)
echo     3. Test with: python test_synthetic.py
echo     4. To build with neural support, install LibTorch to C:\libtorch
echo.
echo   To push:
